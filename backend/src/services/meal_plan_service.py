...
class MealPlanService:
    ...
    async def create_meal_plan(
        ...
        if selected_meal_types is not None:
            ...
            if not required_types.issubset(selected_set):
                ...
                raise ValueError(
                    f"selected_meal_types must include at least: {', '.join(sorted(required_types))}. Missing: {', '.join(sorted(missing))}"
                )
            ...
            if invalid_types:
                raise ValueError(
                    f"Invalid meal types: {', '.join(sorted(invalid_types))}. Valid types: {', '.join(sorted(valid_types))}"
                )
        ...
        try:
            ...
            try:
                ...
                meal_plan.waste_reduction_percentage = waste_metrics.get(
    ...
    async def _calculate_waste_metrics(
        ...
        try:
                ...
                if waste_g > 0:
                    ...
                    if product.perishability in ("stable", "shelf_stable"):
                        ...
                        shelf_stable_items_for_pantry.append(
                            {"product_id": str(product.id), "quantity_g": waste_g}
                        )
                    else:
            ...
            if shelf_stable_items_for_pantry:
                ...
                logger.info(
                    f"Added {len(shelf_stable_items_for_pantry)} shelf-stable items to pantry"
                )
    ...
    async def _calculate_baseline_waste(
        db: AsyncSession, meal_plan: MealPlan, grocery_list, perishable_only: bool = False
    ) -> float:
        """
    ...
    async def _update_pantry_with_leftovers(
        db: AsyncSession, user_id: UUID, leftover_items: List[Dict[str, Any]]
    ) -> None:
        """
            ...
            try:
                ...
                if existing:
                    ...
                    new_item = UserPantryItem(
                        user_id=user_id, product_id=product_id, quantity_g=leftover_qty
                    )
                    db.add(new_item)

                logger.info(
                    f"Added {leftover_qty}g of product {product_id} to pantry for user {user_id}"
                )
    ...
    async def delete_meal_plan(
        ...
        Raises:
            ValueError: If the plan is active and cannot be deleted
        """
        ...
        # Prevent deletion of active plans - must cancel first
        if meal_plan.execution_status == "active":
            raise ValueError("Cannot delete an active plan. Please cancel it first.")
    ...
    async def _fetch_user_pantry(db: AsyncSession, user_id) -> List[Dict[str, Any]]:
        ...
        return [
            {"product_id": str(item.product_id), "quantity_g": float(item.quantity_g)}
