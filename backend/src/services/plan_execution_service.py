...
class PlanExecutionService:
    ...
    async def activate_plan(
        ...
        for item in grocery_list.items:
            ...
            pantry_stmt = select(UserPantryItem).where(
                ...
                UserPantryItem.product_id == item.product_id
            )
            ...
            if pantry_item:
                ...
                pantry_item.quantity_g = float(pantry_item.quantity_g) + float(item.required_quantity_g)
            else:
    ...
    async def complete_meal(
        ...
        existing_stmt = select(MealCompletion).where(
            MealCompletion.meal_id == meal_id,
            MealCompletion.user_id == user_id
        )
        ...
        for ing in ingredients:
            ...
            pantry_stmt = select(UserPantryItem).where(
                UserPantryItem.user_id == user_id,
                UserPantryItem.product_id == ing.product_id
            )
            pantry_result = await db.execute(pantry_stmt)
            pantry_item = pantry_result.scalar_one_or_none()
    ...
    async def uncomplete_meal(
        ...
        Raises:
            ValueError: If completion not found or too old
        """
        ...
        hours_since = (datetime.utcnow() - completion.completed_at).total_seconds() / 3600
        if hours_since > 24:
            raise ValueError("Cannot uncomplete meal after 24 hours")
    ...
    async def get_today_meals(
        ...
        daily_menu_stmt = select(DailyMenu).where(
            DailyMenu.meal_plan_id == plan_id,
            DailyMenu.day_number == current_day
        )
        ...
        for meal in meals:
            completion_stmt = select(MealCompletion).where(
                MealCompletion.meal_id == meal.id,
                MealCompletion.user_id == user_id
            )
